import runebaseJsLib from 'runebasejs-lib'
import btcApp from '@ledgerhq/hw-app-btc'
import transportU2f from '@ledgerhq/hw-transport-u2f'
import BigNumber from 'bignumber.js'
import SafeBuffer from 'safe-buffer'
import OPS from 'runebase-opcodes'
...
export default class Ledger {
  ...
  static async connect() {
    const runebase = new btcApp(await transportU2f.create())
    // ensure in Runebase App
    const pubkeyRes = await runebase.getWalletPublicKey(Ledger.defaultPath)
    if (pubkeyRes.bitcoinAddress[0] !== 'Q') {
      throw 'Not Runebase App'
    }
    return new Ledger(runebase)
  }
  ...
  static async generateTx(keyPair, ledger, path, from, to, amount, fee, utxoList, rawTxFetchFunc = () => {}) {
    ...
    const pubkeyRes = await ledger.runebase.getWalletPublicKey(path)
    ...
    const inputs = []
    const paths = []
    const selectUtxo = runebaseJsLib.utils.selectTxs(utxoList, amount, fee)
    const rawTxCache = {}
    for(let i = 0; i < selectUtxo.length; i++) {
      ...
      inputs.push([
        await ledger.runebase.splitTransaction(rawTxCache[item.hash]),
        item.pos
      ])
    }
    const outputs = new runebaseJsLib.TransactionBuilder(keyPair.network)
    ...
    return await ledger.runebase.createPaymentTransactionNew(inputs, paths, undefined, outputsScript)
  }
  ...
  static async generateSendToContractTx(keyPair, ledger, path, from, contractAddress, encodedData, gasLimit, gasPrice, fee, utxoList, rawTxFetchFunc = () => {}) {
    ...
    const outputs = new runebaseJsLib.TransactionBuilder(keyPair.network)
    const contract =  runebaseJsLib.script.compile([
      OPS.OP_4,
